...
def mavlinkrouter():
    os.system('mavproxy.py --master=/dev/ttyACM0,57600 --out=udpbcast:169.254.148.167:14550')  # Yer Istasyonu IP
    ...
    try:
        ...
        if time.time() - sure > 0.1:
            ...
            if iha.servo6 > 1600 and (iha.servo7 > 1400 and iha.servo7 < 1600):  # ch6: High, ch8: Mid
                mod = "kilitlenme"
                mesaj['iha_otonom'] = 1
                time_difference_value_FBWA = 2

                if iha.pos_alt_rel <= 40:
                    if iha.get_ap_mode() != 'RTL':
                        iha.set_ap_mode('RTL')
                else:
            ...
            if iha.servo6 > 1600 and iha.servo7 < 1400:  # ch6: High, ch8: LOW
                ...
                try:
                    ...
                    if not qr_gidiyor and not kalkista and qr_mesafe > 0.15:  # and iha.pos_alt_rel > 100:
                        ...
                        if iha.get_ap_mode() != "GUIDED":
                            iha.set_ap_mode("GUIDED")
                        qr_git = LocationGlobalRelative(qr_enlem, qr_boylam, 80)
                        iha.set_rc_channel(3, 1500)
                        iha.goto(qr_git)
                        qr_gidiyor = True
                    ...
                    if kalkista and iha.pos_alt_rel < 80:
                        print("kalkiyor")
                        iha.set_rc_channel(1, 1500)
                        iha.set_rc_channel(2, 1900)
                        iha.set_rc_channel(3, 1600)
