...
class Telemetry:
    ...
    def telemetry_packet(self):
        try:
            ...
            gps_saati_unix = (self.telemetry_data['SYSTEM_TIME']['time_unix_usec'] / 1e6
                              if self.telemetry_data['SYSTEM_TIME'] and 'time_unix_usec' in self.telemetry_data['SYSTEM_TIME']
                              else None)
            gps_saati_formatted = self.unix_to_datetime(gps_saati_unix) if gps_saati_unix else None

            telemetry_output = {
                "takim_numarasi": 1,
                "iha_enlem": self.telemetry_data['GPS_RAW_INT']['lat'] / 1e7
                if self.telemetry_data['GPS_RAW_INT'] and 'lat' in self.telemetry_data['GPS_RAW_INT']
                else None,
                "iha_boylam": self.telemetry_data['GPS_RAW_INT']['lon'] / 1e7
                if self.telemetry_data['GPS_RAW_INT'] and 'lon' in self.telemetry_data['GPS_RAW_INT']
                else None,
                "iha_irtifa": self.telemetry_data['VFR_HUD']['alt']
                if self.telemetry_data['VFR_HUD'] and 'alt' in self.telemetry_data['VFR_HUD']
                else None,
                "iha_dikilme": self.telemetry_data['VFR_HUD']['pitch']
                if self.telemetry_data['VFR_HUD'] and 'pitch' in self.telemetry_data['VFR_HUD']
                else None,
                "iha_yonelme": self.telemetry_data['VFR_HUD']['heading']
                if self.telemetry_data['VFR_HUD'] and 'heading' in self.telemetry_data['VFR_HUD']
                else None,
                "iha_yatis": self.telemetry_data['VFR_HUD']['roll']
                if self.telemetry_data['VFR_HUD'] and 'roll' in self.telemetry_data['VFR_HUD']
                else None,
                "iha_hiz": self.telemetry_data['VFR_HUD']['airspeed']
                if self.telemetry_data['VFR_HUD'] and 'airspeed' in self.telemetry_data['VFR_HUD']
                else None,
                "iha_batarya": self.telemetry_data['SYS_STATUS']['battery_remaining']
                if self.telemetry_data['SYS_STATUS'] and 'battery_remaining' in self.telemetry_data['SYS_STATUS']
                else None,
                "iha_otonom": 1,
                "iha_kilitlenme": 1,
                "hedef_merkez_X": 300,
            ...
            print(f"Error in telemetry packet: {e}")
            return [self.telemetry_data, self.simplified_telemetry_data]

"""if __name__ == "__main__":
    telemetry = Telemetry("10.80.1.46")
