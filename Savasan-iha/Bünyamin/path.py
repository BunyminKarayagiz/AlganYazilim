...
class Plane():
    ...
    def _setup_listeners(self):  # -- (private) Set up listeners
        ...
        if True:
            ...
            def listener(vehicle, name, message):
                ...
                self.servo8 = message.servo8_raw
    ...
    def mission_add_takeoff(self, takeoff_altitude=50, takeoff_pitch=15, heading=None):
        ...
        if is_mission and tmp_mission[0].command == mavutil.mavlink.MAV_CMD_NAV_TAKEOFF:
            ...
            self.clear_mission()
            takeoff_item = Command(0, 0, 0, 3, mavutil.mavlink.MAV_CMD_NAV_TAKEOFF, 0, 0, takeoff_pitch, 0, 0, heading,
    ...
    def mesajlar(self):
        self.mesaj = {
            ...
            "hedef_yukseklik": 0,
            "gps_saati": {
                "saat": self.gps_time.hour,  # datetime.datetime.now().hour,
                "dakika": self.gps_time.minute,  # datetime.datetime.now().minute,
                "saniye": self.gps_time.second,  # datetime.datetime.now().second,
                "milisaniye": self.gps_time.microsecond // 1000,  # int(datetime.datetime.now().microsecond//1000)
            }
        }
        return self.mesaj
