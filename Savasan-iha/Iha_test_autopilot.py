...
class client_manager:
    def __init__(self,yazilim_ip,yonelim_ip) -> None:
        ...
        self.TCP_KAMIKAZE_TIME=Client_Tcp.Client(HOST=yazilim_ip,PORT=8005,name="KAMIKAZE_TIME")
    ...
    def CONNECT_TIME_KAMIKAZE_CLIENT(self):
        connection=False
        while not connection:
            try:
                self.TCP_KAMIKAZE_TIME.connect_to_server()
                connection=True
                print("KAMIKAZE_TIME SERVER: BAĞLANDI.")
            except (ConnectionError , Exception) as e:
                print("KAMIKAZE_TIME SERVER: baglanırken hata: ", e)
            time.sleep(1)
        self.KAMIKAZE_TIME_SERVER_STATUS=connection
        return self.TCP_KAMIKAZE_TIME
    ...
    def connect_to_servers(self):
        ...
        th4=threading.Thread(target=self.CONNECT_KAMIKAZE_CLIENT) #wait_for_client içinde kullanıldı.
        th5=threading.Thread(target=self.CONNECT_CONFIRMATION_CLIENT)
        th6=threading.Thread(target=self.CONNECT_TIME_KAMIKAZE_CLIENT)
...
class autopilot:
    ...
    def throttle_set(self,ch_val):
        self.TUYGUN_PIXHAWK.set_rc_channel(self.throttle_channel,ch_val)
    ...
    def pitch_stabilize(self,pitch):
        ...
        #while not (pitch < 10 and pitch > -10) :
        if (pitch < 5 and pitch > 1 ) :
            self.pitch_set(ch_val = pitch)
            pitch=self.roll_check()
        else:
            print("ROLL STABILIZED -- ROLL STABILIZED -- ROLL STABILIZED -- ROLL STABILIZED -- ROLL STABILIZED")

    def gain_alt_maneuver(self,alt):
        pass
    ...
    def trigger_failsafe(self):
        while True:
            roll_value = self.roll_check()
            pitch_value = self.pitch_check()
            altitude_value = self.alt_check()
            stall_diff,speed_value = self.stall_check()
            print(f"roll:{roll_value} -- pitch:{pitch_value} -- alt:{altitude_value} -- stall_diff:{stall_diff} ")

            if altitude_value < self.critical_altitude:
                self.altitude_warning = True
                print(f"ALTITUDE WARNING!!! {self.altitude_warning}")
            else:
                self.altitude_warning = False
            if roll_value > self.roll_limit and roll_value < -1*self.roll_limit:
                self.roll_warning = True
                print(f"ROLL WARNING!!! {self.roll_warning}")
            else:
                self.roll_warning = False
            if pitch_value > self.pitch_limit and pitch_value < -1*self.pitch_limit:
                self.pitch_warning = True
                print(f"PITCH WARNING!!! {self.pitch_warning}")
            else:
                self.pitch_warning = False
            if stall_diff < self.critical_stall_diff:
                self.stall_warning = True
                print(f"STALL WARNING!!!   {self.stall_warning}")
            else:
                self.stall_warning = False

            if self.YKI_CONFIRMATION_STATUS:
                ...
                if self.roll_warning and self.pitch_warning and self.altitude_warning and self.stall_warning:
                    print("gg wp - Yeni gövde siparişi")
                elif self.roll_warning and self.pitch_warning and self.altitude_warning:
                    pass
                elif self.roll_warning and self.pitch_warning:
                    pass
                


            else:
                print("FAILSAFE UNAUTHORIZED... NEED GCS CONFIRMATION")
    ...
    def KAMIKAZE(self):
        if self.qr_location != None:
            qr_boylam = self.qr_location['qrBoylam']
            qr_enlem = self.qr_location['qrEnlem']
            if self.CLIENT_MANAGER.YKI_CONFIRMATION_STATUS:
                ...
                if qr_mesafe < 0.8:
                    ...
                    if self.TUYGUN_PIXHAWK.pos_alt_rel >=80 and self.TUYGUN_PIXHAWK.pos_alt_rel < 100 and self.kamikaze_dive_state and not self.kamikaze_recover_state:
                        ...
                        if self.kamikaze_clock:
                            self.kamikaze_end_time = datetime.datetime.fromtimestamp(time.time(), pytz.timezone('UTC'))
                            self.kamikaze_clock=False
                        packet={
                                    "kamikazeBaslangicZamani"
                                    : {"saat": self.kamikaze_start_time.hour,
                                    "dakika": self.kamikaze_start_time.minute,
                                    "saniye": self.kamikaze_start_time.second,
                                    "milisaniye": self.kamikaze_start_time.microsecond / 1000
                                    },
                                    "kamikazeBitisZamani":
                                    {"saat": self.kamikaze_end_time.hour,
                                    "dakika": self.kamikaze_end_time.minute,
                                    "saniye": self.kamikaze_end_time.second,
            ...
            print("QR-COORD NOT AVAILABLE -- QR-COORD NOT AVAILABLE -- QR-COORD NOT AVAILABLE -- QR-COORD NOT AVAILABLE")
    ...
    def AUTOPILOT_STATE_CONTROL(self):
        ...
        while True:
            if (not self.FAILSAFE_TAKEOVER) and self.CLIENT_MANAGER.YKI_CONFIRMATION_STATUS: #Failsafe-LOW , CONFIRMATION-HIGH
                ...
                if self.current_mode == "AUTO" and self.previous_mode != "AUTO":
                    if self.TUYGUN_PIXHAWK.get_ap_mode() != "AUTO":
                        self.TUYGUN_PIXHAWK.set_ap_mode("AUTO")
                        self.previous_mode = "AUTO"

                elif self.current_mode == "FBWA" and self.previous_mode != "FBWA":
                    ...
                    if self.TUYGUN_PIXHAWK.get_ap_mode() != "FBWA":
                        self.TUYGUN_PIXHAWK.set_ap_mode("FBWA")
                        self.previous_mode = "FBWA"

                elif self.current_mode == "RTL" and self.previous_mode != "RTL":
                    print(f"SELECTED MODE : {self.current_mode} SELECTED MODE : {self.current_mode} SELECTED MODE : {self.current_mode} SELECTED MODE : {self.current_mode} SELECTED MODE : {self.current_mode}")
                    if self.TUYGUN_PIXHAWK.get_ap_mode() != "RTL":
                        self.TUYGUN_PIXHAWK.set_ap_mode("RTL")
                        self.previous_mode = "RTL"
            ...
            elif (self.FAILSAFE_TAKEOVER) and (not self.CLIENT_MANAGER.YKI_CONFIRMATION_STATUS): #Failsafe-HIGH , CONFIRMATION-LOW
