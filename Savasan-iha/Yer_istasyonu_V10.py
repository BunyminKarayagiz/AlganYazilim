            ...
            if self.MODE_SERVER_STATUS:
                try:
                    ...
                    try:
                        ...
                        if self.secilen_görev_modu == "KILITLENMEKILITLENMEKILITLENME" and ( not self.önceki_mod=="KILITLENMEKILITLENMEKILITLENME"):
                            self.trigger_event("Frame_1","kilitlenme")
                            self.trigger_event("Frame_2","kilitlenme")
                            self.önceki_mod = "KILITLENMEKILITLENMEKILITLENME"
                            self.iha_mod = 1
                            cp.info(f"GOREV MODU : Degisim -> {self.secilen_görev_modu}")

                        elif self.secilen_görev_modu == "KAMIKAZEKAMIKAZEKAMIKAZE" and not (self.önceki_mod=="KAMIKAZEKAMIKAZEKAMIKAZE"):
                            self.trigger_event("Frame_1","kamikaze")
                            self.trigger_event("Frame_2","kamikaze")
                            self.önceki_mod = "KAMIKAZEKAMIKAZEKAMIKAZE"
                            self.iha_mod = 1
                            cp.info(f"GOREV MODU : Degisim -> {self.secilen_görev_modu}")

                        elif self.secilen_görev_modu == "AUTOAUTOAUTO" and not (self.önceki_mod=="AUTOAUTOAUTO"):
                            self.trigger_event("Frame_1","AUTO")
                            self.trigger_event("Frame_2","AUTO")
                            self.önceki_mod = "AUTOAUTOAUTO"
                            self.iha_mod = 1
                            cp.info(f"GOREV MODU : Degisim -> {self.secilen_görev_modu}")

                        elif self.secilen_görev_modu == "FBWAFBWAFBWA" and not (self.önceki_mod=="FBWAFBWAFBWA"):
                            self.trigger_event("Frame_1","FBWA")
                            self.trigger_event("Frame_2","FBWA")
                            self.önceki_mod = "FBWAFBWAFBWA"
                            self.iha_mod = 0
                            cp.info(f"GOREV MODU : Degisim -> {self.secilen_görev_modu}")

                        elif self.secilen_görev_modu == "RTLRTLRTL" and not (self.önceki_mod=="RTLRTLRTL"):
                            self.trigger_event("Frame_1","RTL")
                            self.trigger_event("Frame_2","RTL")
                            self.önceki_mod = "RTLRTLRTL"
                            self.iha_mod = 1
                            cp.info(f"GOREV MODU : Degisim -> {self.secilen_görev_modu}")
