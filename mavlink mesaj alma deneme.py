    ...
    def veri_kaydetme(self):
        while True:
            ...
            if self.msg.get_type() == 'GPS_RAW_INT':
                ...
                self.roll = self.msg.roll  *90/sp.pi
                self.pitch = self.msg.pitch *90/sp.pi
                self.yaw = self.msg.yaw *90/sp.pi
            ...
            self.telemetri = {
                "takim_numarasi": 1,
                "iha_enlem": float("{:.12f}".format(self.enlem)),
                "iha_boylam": float("{:.12f}".format(self.boylam)),
                "iha_irtifa": float("{:.2f}".format(self.yukseklik)),
                "iha_pitch": float("{:.2f}".format(self.pitch)),
                "iha_yaw": float("{:.2f}".format(self.yaw)),
                "iha_roll": float("{:.2f}".format(self.roll)),
            ...
            while True:
                if time.time() - self.gonderilen_zaman < 0.25:
                    break
                else:
